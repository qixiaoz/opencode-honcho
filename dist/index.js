...
const resolveHonchoCtor = (sdk) => {
    if (!sdk || typeof sdk !== "object") {
        throw new Error("Honcho SDK constructor is unavailable");
    }
    const direct = sdk.Honcho;
    if (typeof direct === "function") {
        return direct;
    }
    const fallback = sdk.default;
    if (fallback) {
        return resolveHonchoCtor(fallback);
    }
    throw new Error("Honcho SDK constructor is unavailable");
};
let honchoCtorPromise = null;
...
const normalizeId = (value) => trimHyphenEdges(value.toLowerCase().replace(/[^a-z0-9_-]+/g, "-")) || "default";
...
const normalizedRawSettings = (raw) => {
    const normalized = { ...raw };
    const persistedApiKey = typeof raw[PERSISTED_API_KEY_FIELD] === "string"
        ? expandEnv(raw[PERSISTED_API_KEY_FIELD])
        : "";
    const legacyApiKey = typeof raw[LEGACY_API_KEY_FIELD] === "string"
        ? expandEnv(raw[LEGACY_API_KEY_FIELD])
        : "";
...
const readConfigFile = async (configPath) => {
    try {
        return normalizedRawSettings(JSON.parse(await readFile(configPath, "utf-8")));
    }
    catch (error) {
...
const sessionPeerAdditions = (topology) => Object.entries(topology.sessionPeerConfigs).map(([peerId, config]) => [peerId, config]);
...
const toUserFacingPeerDescription = (peer) => {
    if (!peer) {
        return null;
    }
    return {
