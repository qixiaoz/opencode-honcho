...
const createPluginHarness = async (rootDir) => {
  ...
  return hooks.tool.honcho_setup
}
...
test(
  "honcho_setup switches a persisted local baseUrl back to cloud when only an apiKey is provided",
  { concurrency: false },
  async () => {
  ...
  await withEnv({ XDG_CONFIG_HOME: configHome }, async () => {
    const honchoSetup = await createPluginHarness(rootDir)
    ...
    assert.deepEqual(result.persistedFields.sort(), ["apiKey", "baseUrl"])
  })
  },
)
